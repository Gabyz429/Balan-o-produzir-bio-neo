...
.badge {display:inline-block;padding:2px 8px;border:1px solid #e5e7eb;border-radius:999px;background:#fafafa;font-size:.75rem;margin-left:6px}
...
with st.sidebar:
    ...
    moagem_por_dia = st.number_input("Moagem (t/dia) enviada", min_value=0.0, value=35.0, step=1.0, format="%.3f")
    dias_bio = st.number_input("Dias em produção na Bio", min_value=0, value=25, step=1)
...
with tab1:
    ...
    with col1:
        vazao_vinho_m3h = st.number_input("Vazão do vinho (m3/h)", min_value=0.0, value=100.0, step=1.0, format="%.2f")
    with col2:
        perc_ds_vinho = st.number_input("%Ds (vinho)", min_value=0.0, value=8.5, step=0.1, format="%.2f")
    with col3:
        conc_vinho_ww = st.number_input("Conc. vinho w/w (%%)", min_value=0.0, value=14.5, step=0.1, format="%.2f")
    ...
    with c1:
        fator_vinho_para_etanol = st.number_input("Fator vinho→etanol (m3 etanol / m3 vinho)", min_value=0.0, value=0.1931, step=0.0001, format="%.4f", help="Ajuste para refletir seu balanço real. Ex.: 100 m3/h vinho → 19,31 m3/h etanol ⇒ 0,1931")
    with c2:
        rel_vinhaca_sobre_etanol = st.number_input("Relação vinhaça/etanol (m3/m3)", min_value=0.0, value=6.0, step=0.1, format="%.2f")
    with c3:
        perc_ds_vinhaca = st.number_input("%Ds (vinhaça)", min_value=0.0, value=7.34, step=0.01, format="%.2f")
...
with tab2:
    ...
    c1, c2, c3 = st.columns(3)
    with c1:
        etanol_m3h_dir = st.number_input("Etanol hidratado (m3/h)", min_value=0.0, value=19.31, step=0.01, format="%.2f")
    with c2:
        rel_vinhaca_sobre_etanol_dir = st.number_input("Relação vinhaça/etanol (m3/m3)", min_value=0.0, value=6.0, step=0.1, format="%.2f")
    with c3:
        perc_ds_vinhaca_dir = st.number_input("%Ds (vinhaça)", min_value=0.0, value=7.34, step=0.01, format="%.2f")
...
with st.sidebar:
    ...
    preco_oleo_rpt = st.number_input("Óleo - preço líquido", min_value=0.0, value=5000.0, step=50.0, format="%.2f")
    preco_ddgs_rpt = st.number_input("DDGS - preço líquido", min_value=0.0, value=1000.0, step=50.0, format="%.2f")
    preco_etanol = st.number_input("Etanol (R$/t ou m3)", min_value=0.0, value=2800.0, step=50.0, format="%.2f")

    st.header("🧪 Rendimentos (por t de moagem)")
    rendimento_oleo_kgpt = st.number_input("Óleo (kg/t)", min_value=0.0, value=19.0, step=0.1, format="%.2f")
    rendimento_ddgs_kgpt = st.number_input("DDGS (kg/t)", min_value=0.0, value=250.0, step=1.0, format="%.1f")

    st.header("🍶 Etanol produzido na Bio (período)")
    modo_etanol = st.radio("Como informar etanol?", ["Capacidade por dia × dias", "Total direto"], index=0)
    if modo_etanol == "Capacidade por dia × dias":
        ...
        etanol_total = st.number_input("Etanol total no período (t ou m3)", min_value=0.0, value=11588.666, step=1.0, format="%.3f")
...
st.divider()
st.caption("Observações: • App ignora custos/margens de vapor. • Unidades de etanol (t ou m3) devem ser consistentes com o preço informado. • Ajuste fatores para refletir seu balanço real.")
